...
export const StudentDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const handleRecoveryEmailVerified = () => setRecoveryEmailVerified(true);
  ...
  const downloadNoticeFile = async (notice: Announcement) => {
    ...
    anchor.href = url;
    anchor.download = notice.attachment_name || 'course-file';
    anchor.click();
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {recoveryEmailVerified === false && !recoveryTipSkipped && (
        <section className="uipro-card relative overflow-hidden rounded-2xl border border-blue-200 bg-blue-50/90 p-5 shadow-sm dark:border-blue-500/30 dark:bg-blue-500/10">
          <div className="pointer-events-none absolute -right-10 -top-12 h-32 w-32 rounded-full bg-blue-200/40 blur-2xl dark:bg-blue-500/15" />
          <button
            type="button"
            onClick={() => setRecoveryTipSkipped(true)}
            className="absolute right-3 top-3 rounded-lg p-1.5 text-slate-400 transition hover:bg-white hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500/30 dark:hover:bg-slate-800 dark:hover:text-slate-200"
            aria-label="Skip account setup for now"
          >
            <X className="h-4 w-4" />
          </button>

          <div className="relative flex flex-col gap-4 pr-7 md:flex-row md:items-center md:justify-between">
            <div className="flex min-w-0 gap-3.5">
              ...
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="text-sm font-extrabold text-slate-900 dark:text-white">Finish account setup</h3>
                  <span className="rounded-full bg-amber-100 px-2.5 py-1 text-[9px] font-extrabold uppercase tracking-wide text-amber-700 dark:bg-amber-500/15 dark:text-amber-300">
                    1 task left
                  </span>
                </div>
                <p className="mt-1 text-xs font-medium leading-relaxed text-slate-600 dark:text-slate-300">
                  Add and verify your personal Gmail for password resets.
                </p>
                <div className="mt-2 flex items-center gap-2 text-[10px] font-bold text-blue-700 dark:text-blue-300">
                  <span className="flex h-5 w-5 items-center justify-center rounded-full bg-blue-600 text-white">1</span>
                  Open Profile, then add your Gmail.
                </div>
              </div>
            </div>

            <div className="flex shrink-0 items-center gap-2 pl-[3.4rem] md:pl-0">
              <button
                type="button"
                onClick={() => setRecoveryTipSkipped(true)}
                className="h-10 rounded-xl px-4 text-xs font-bold text-slate-600 transition hover:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/25 dark:text-slate-300 dark:hover:bg-slate-800"
              >
                Skip for now
              </button>
              <button
                type="button"
                onClick={() => window.dispatchEvent(new CustomEvent('open-user-profile', { detail: { tab: 'security' } }))}
                className="inline-flex h-10 items-center gap-2 rounded-xl bg-blue-600 px-4 text-xs font-bold text-white shadow-sm transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:ring-offset-2 dark:focus:ring-offset-slate-950"
              >
                Start setup
      ...
      {announcements.length > 0 && (
        <section className="uipro-card rounded-2xl border border-slate-200 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900/60">
          <div className="mb-4 flex items-center gap-2 border-b border-slate-100 pb-3 dark:border-slate-800">
            <Bell className="h-4 w-4 text-blue-600 dark:text-blue-400" />
            <div><h3 className="text-sm font-extrabold text-slate-900 dark:text-slate-100">Latest Notices</h3><p className="text-[10px] text-slate-500 dark:text-slate-400">General and course updates in one place.</p></div>
          </div>
          <div className="grid gap-3 lg:grid-cols-2">
            {announcements.slice(0, 6).map(notice => {
              const isCourse = notice.target_scope === 'course';
              return <article key={notice.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  <span className={`rounded-full px-2.5 py-1 text-[9px] font-extrabold uppercase ${isCourse ? 'bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-300' : 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300'}`}>{isCourse ? 'Course' : 'General'}</span>
                  {isCourse && <span className="text-[9px] font-bold text-slate-500 dark:text-slate-400">{notice.target_course_code} · {notice.target_group?.toLowerCase() === 'all' ? 'All groups' : notice.target_group}</span>}
                </div>
                <h4 className="text-xs font-extrabold text-slate-900 dark:text-slate-100">{notice.title}</h4>
                <p className="mt-1 whitespace-pre-wrap text-[11px] leading-relaxed text-slate-600 dark:text-slate-300">{notice.content}</p>
                {(notice.attachment_name || notice.external_link) && <div className="mt-3 flex flex-wrap gap-2">
                  {notice.attachment_name && <button type="button" onClick={() => downloadNoticeFile(notice)} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 px-2.5 py-1.5 text-[10px] font-bold text-blue-700 transition hover:bg-blue-100 dark:bg-blue-500/10 dark:text-blue-300"><Download className="h-3.5 w-3.5" />{notice.attachment_name}</button>}
                  {notice.external_link && <a href={notice.external_link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1.5 text-[10px] font-bold text-slate-700 transition hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-200"><ExternalLink className="h-3.5 w-3.5" />Open link</a>}
                </div>}
              </article>;
            })}
          </div>
