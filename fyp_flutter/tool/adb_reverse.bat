@echo off
REM ---------------------------------------------------------------
REM Applies `adb reverse tcp:8003 tcp:8003` to EVERY connected device
REM so the phone's localhost:8003 tunnels to the PC's backend.
...
for /f "skip=1 tokens=1,2" %%a in ('"%ADB%" devices') do (
  if "%%b"=="device" (
    "%ADB%" -s %%a reverse tcp:8003 tcp:8003 >nul 2>&1
    echo   [OK] %%a  localhost:8003 -^> PC:8003
  )
)
echo Done.
exit /b 0
